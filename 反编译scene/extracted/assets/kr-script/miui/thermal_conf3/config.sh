...
install_thermal() {
  ...
  # 检查其它模块是否更改温控
  modules=$(dirname $MAGISK_PATH)
  for module in `ls $modules`; do
    if [[ "$modules/$module" != "$MAGISK_PATH" && -d "$modules/$module" ]] && [[ ! -f "$modules/$module/disable" ]] && [[ ! -f "$modules/$module/remove" ]]; then
      if [[ "$module" != uperf ]] && [[ "$module" != *extreme_gt* ]] && [[ -d "$modules/$module/system" ]]; then
        find_result=`find "$modules/$module/system" -name "*thermal*" -type f`
