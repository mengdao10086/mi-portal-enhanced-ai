...
mkdir -p $module
SRC=$PAGE_WORK_DIR/extreme_gt_flyme
cat $SRC/module.prop > $module/module.prop
cat $SRC/module_uninstall.sh > $module/uninstall.sh
cat $SRC/service.sh > $module/service.sh
cat $SRC/post-fs-data.sh > $module/post-fs-data.sh
