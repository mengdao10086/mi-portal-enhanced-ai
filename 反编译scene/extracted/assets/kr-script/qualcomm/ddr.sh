path=/sys/devices/system/cpu/bus_dcvs/DDR

lock_value () {
  chmod 644 $2
  echo $1 > $2
  chmod 444 $2
}

get_min_freq(){
  cat $path/*/min_freq | head -1
}
get_max_freq(){
  cat $path/*/max_freq | head -1
}
set_min_freq(){
  for file in $path/*/min_freq
  do
    lock_value $state $file
  done
}
set_max_freq(){
  for file in $path/*/max_freq
  do
    lock_value $state $file
  done
}

get_boost_freq(){
  cat $path/boost_freq
}
set_boost_freq(){
  lock_value $state $path/boost_freq
}

check_ddr_frequency_mhz() {
  debugfs=$(grep -e "^debugfs" /proc/mounts | head -1 | awk '{print $2}')
  if [[ -f $debugfs/qcom_aoss/ddr_frequency_mhz ]]; then
    echo 1
  fi
}
get_ddr_frequency_mhz() {
  cat /dev/ddr_frequency_mhz
}
set_ddr_frequency_mhz() {
  debugfs=$(grep -e "^debugfs" /proc/mounts | head -1 | awk '{print $2}')
  echo $((state/1000)) > $debugfs/qcom_aoss/ddr_frequency_mhz
  echo "$state" > /dev/ddr_frequency_mhz
}

options(){
  for item in $(cat $path/available_frequencies)
  do
    echo $item
  done
}
options_and_0(){
  echo 0
  options
}

visible() {
  if [[ -d $path ]]; then
    echo 1
  else
    echo 0
  fi
}

$1